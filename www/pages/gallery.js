...
(function () {
  ...
  lib.genDivs = function () {
    ...
    lib.topDiv = idv.topbar.genTopbar(outerDiv,topbarOptions);
    ...
    lib.tabGroup = lib.genTab(outerDiv,cb,"Gallery");
  ...
  lib.popChoiceOfAlbum = function (im) {
    var albums = im.albums;
    var ttl = im.title;
    if (!ttl) {
      ttl = util.pathLast(im.topic);
    }
    var msg = $('<div/>');
    msg.css({'margin':'30px'});
    msg.append('<div>Albums (sets of annotations of <i>'+ttl+'</i>):</div>');
    util.arrayForEach(albums,function (a) {
      ...
      var dst = 'http://s3.imagediver.org/topic'+(a.topic)+'/index.html';
  ...
  lib.addPic = function (im) {
    ...
    */
    var tp = im.topic;
    var albs = im.albums;
    var acnt = albs.length;//im.albumCount
    ...
    */
    ...
    /*if (acnt == 1) {
      ...
      imdiv.click(function () {lib.popChoiceOfAlbum(im);});
    }
    */
    imdiv.append(cntel);
  ...
  lib.initialize = function() {
    ...
    util.commonInit();
    util.post(url,{includeAlbums:1,featuredOnly:1},function (rs) {
