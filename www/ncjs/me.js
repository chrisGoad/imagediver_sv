(function(){var lib=page;var geom=idv.geom;var imlib=idv.image;var common=idv.common;var util=idv.util;function addButton(cn,txt,cb,inDiv){var bt=$('<span class="clickableElement"></span>');bt.html(txt);if(inDiv){var cnt=$('<div/>');cn.append(cnt);var rs=cnt;}else{cnt=cn;rs=bt;}
...
lib.initialize=function(options){if(options.timed_out){$('.infoDiv').html("Your session has timed out. Please sign in again, if you like.");util.logout();return;}
var jsonUrl='/api/albumsAndImages'
util.commonInit()
idv.topbar.genTopbar($('body'),{title:'Your Account'});theUser.userId=options.userId
theUser.name=options.name
theUser.email=options.email
theUser.tumblr_name=options.tumblr_name;var pdiv=$('.infoDiv');var nameEl,uidEl;var name=$.trim(options.name);var nameDiv,emailDiv,tumblrDiv;pdiv.append(nameDiv=$('<div/>').html(name).css({"margin-bottom":"30pt","font-weight":"bold"}));if(options.tumblr_name){pdiv.append(tumblrDiv=$('<div/>').html("Logged in with Tumblr").css({"margin-bottom":"30pt"}));}
if(options.twitter_name){pdiv.append(tumblrDiv=$('<div/>').html("Logged in with Twitter").css({"margin-bottom":"30pt"}));}
