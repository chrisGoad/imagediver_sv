...
var exports={};var GEOM2D={};exports.GEOM2D=GEOM2D;(function(){var lib=GEOM2D;lib.Point=function(x,y){this.x=x;this.y=y;}
...
lib.Rect.prototype.upperLeft_new=function(p){var cc=this.corner;var d=cc.minus(p);var xt=this.extent;var nxt=xt.plus(d);return new lib.Rect(p,nxt);}
lib.Rect.prototype.middleLeft=function(){var p=this.corner;var ht=this.height();return p.plusY(0.5*ht);}
lib.Rect.prototype.middleLeft_new=function(p){var cc=this.corner;var dx=cc.x-p.x;var xt=this.extent;var nxt=new lib.Point(xt.x+dx,xt.y);var ncr=new lib.Point(p.x,cc.y);return new lib.Rect(ncr,nxt);}
lib.Rect.prototype.lowerLeft=function(){var p=this.corner;var ht=this.height();return p.plusY(ht);}
lib.Rect.prototype.lowerLeft_new=function(p){var ll=this.lowerLeft();var cc=this.corner;var d=new lib.Point(ll.x-p.x,p.y-ll.y);var xt=this.extent;var nxt=xt.plus(d);var ncr=new lib.Point(p.x,cc.y);return new lib.Rect(ncr,nxt);}
lib.Rect.prototype.upperMiddle=function(){var p=this.corner;var wd=this.width();return p.plusX(0.5*wd);}
lib.Rect.prototype.upperMiddle_new=function(p){var cc=this.corner;var dy=cc.y-p.y;var xt=this.extent;var nxt=new lib.Point(xt.x,xt.y+dy);var ncr=new lib.Point(cc.x,p.y);return new lib.Rect(ncr,nxt);}
...
lib.Rect.prototype.lowerMiddle_new=function(p){var cc=this.corner;var ll=this.lowerLeft();var dy=p.y-ll.y;var xt=this.extent;var nxt=new lib.Point(xt.x,xt.y+dy);return new lib.Rect(cc,nxt);}
lib.Rect.prototype.upperRight=function(){var p=this.corner;var wd=this.width();return p.plusX(wd);}
lib.Rect.prototype.upperRight_new=function(p){var ur=this.upperRight();var cc=this.corner;var d=new lib.Point(p.x-ur.x,ur.y-p.y);var xt=this.extent;var nxt=xt.plus(d);var ncr=new lib.Point(cc.x,p.y);return new lib.Rect(ncr,nxt);}
...
lib.Rect.prototype.middleRight_new=function(p){var cc=this.corner;var xt=this.extent;var dx=p.x-(cc.x+xt.x);var nxt=new lib.Point(xt.x+dx,xt.y);return new lib.Rect(cc,nxt);}
lib.Rect.prototype.lowerRight=function(){return this.corner.plus(this.extent);}
lib.Rect.prototype.lowerRight_new=function(p){var rp=this.lowerRight();var cc=this.corner;var d=new lib.Point(p.x-rp.x,p.y-rp.y);var xt=this.extent;var nxt=xt.plus(d);return new lib.Rect(cc,nxt);}
...
var IMAGE={};exports.IMAGE=IMAGE;(function(){var lib=IMAGE;var util=idv.util;var Process;var geom;if(typeof require=="function"){ Process=require("process").Process;geom=require('/var/www/neo.com/lib/geom2d.js').GEOM2D;}else{geom=exports.GEOM2D;}
...
lib.Image=function(imD){var imDimx=imD.dimensions.x;var imDimy=imD.dimensions.y;var imDim=new geom.Point(imDimx,imDimy);this.extent=imDim;var tilingDir=imD.tilingDir;if(tilingDir&&0){this.tilingDir=imD.tilingDir;this.tilingUrl=imD.tilingUrl;}else{ var user=imD.owner;if(!user){user=imD.user;}
var imname=imD.name;var lin=user.lastIndexOf("/");var uname=user.substr(lin+1);this.tilingDir="/mnt/ebs1/imagediver/tilings/"+uname+"/"+imname+"/";var bucket=imD.bucket;if(!bucket){bucket="idv_"+uname;}
...
var imlib=exports.IMAGE;(function(){var lib=exports.IMAGE;idv.imlib=lib;lib.noCanvas=!(Modernizr.canvas);lib.noCanvas=true;if(!lib){exports.IMAGE={};lib=exports.IMAGE;}
var geom=exports.GEOM2D;var util=idv.util;idv.imageLoadCount=0; idv.outlineColor="red";idv.selectedColor="yellow";lib.theDepthOffset=1; lib.theDepthBump=1; lib.resBias=3.0;
...
(function(){var lib=exports.IMAGE; var geom=exports.GEOM2D;var util=idv.util;lib.Viewport.prototype.setPan=function(ip,skipCallbacks){ util.log("setPan","pan",ip.x,ip.y,"maxypan",this.cmaxYpan,"zoom",this.zoom,"depth",this.depth);var lp=this.limitPan(ip);if(lp!=null){this.limitedPan=lp;p=lp;}
...
idv.common={};idv.css.lightbox={border:"white solid",position:"absolute","z-index":2000,"background-color":"#444444","color":"white"};idv.css.lightbox={border:"white solid",position:"absolute","z-index":2000,"background-color":"#999999","color":"black"};abcd=22;(function(){var lib=idv.common;var util=idv.util;lib.genLogo=function(){var rs=$('<span class="logoSpan"/>');var left=$('<span class="logoLeft">image</div>');var right=$('<span class="logoRight">Diver</div>');rs.append(left);rs.append(right);rs.click(function(){location.href="/";});return rs;}
...
lib.closeX='<div class="closeX" style="padding:3px;cursor:pointer;background-color:red;font-weight:bold;border:thin solid white;font-size:12pt;color:white;float:right">X</div>';lib.Lightbox.prototype.addClose=function(){var thisHere=this;this.close=$(lib.closeX);this.close.click(function(){thisHere.dismiss();thisHere.afterClose();});this.element.append(this.close);}
...
var page={};(function(){var lib=page;var geom=exports.GEOM2D;var imlib=exports.IMAGE;var com=idv.common;var util=idv.util;lib.twoColumns=true; 
...
lib.setTopDivCss=function(layout){layout.css.titleDiv={"margin-top":"30px","margin-bottom":"20px","font-size":"10pt","font-style":"bold"};layout.css.titleDiv={"font-size":"10pt","font-style":"bold"};var tdHt=idv.embed?"20px":"35px";layout.css.topDiv={"margin-top":"0px",padding:"0px","margin-bottom":"10px",height:tdHt,"font-size":"10pt","font-style":"bold"};layout.css.logo={"font-size":"12pt"};layout.css.topDivTop={"height":"20px"};}
lib.applyTopDivCss=function(layout){if(!idv.embed)lib.setCss(imlib.titleDiv,layout.css.titleDiv);lib.setCss(imlib.topDiv,layout.css.topDiv);lib.setCss($(".logo"),layout.css.logo);lib.setCss($(".topDivTop"),layout.css.topDivTop);}
