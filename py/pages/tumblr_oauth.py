"""
The callback page for tumblr.
http://dev.imagediver.org/authorize_tumblr
"""
...
bodyText = """
<div style="text-align:center;padding:30px">Accesss to Tumblr was denied.</div>
</div>
"""
...
def emitRedirect(verifier,token):
   ...
   <script type="text/javascript" src="/lib/util.js"></script>
    ...
    (function () {
      ...
      var tkj = sessionStorage.tumblrToken;
      var tk = JSON.parse(tkj);
      tk.verifier = '"""+verifier+"""';
      
      var url = "/api/setTumblrToken";
      if (sessionStorage.signingInWithTumblr) {
        tk.signingIn = 1;
      }
      ...
      idv.util.post(url,data,function (rs) {
        ...
        var dataj = sessionStorage.post_to_tumblr_args;
        if (!dataj) alert("Internal problem: missing session storage data");
        var abc = 22;
         var inp;
        var frm = $('<form enctype = "text/plain" action="/post_to_tumblr" method="POST">').append(
...
def emitPage(webin):
  ...
  return gen.genHtmlPage(webin,"Tumblr access was denied.",styleSheets=styleSheets,pageTitle="Access Denied")

  #return gen.genHtmlPage(webin,jsFiles,headerTitle,headLines,bodyText,styleSheets=styleSheets,pageTitle="Tumblr access was denied.")
