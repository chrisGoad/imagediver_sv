...
python admin_scripts/json_to_s3.py cg The_Dutch_Proverbs 1
...
def saveFile(bucket,user,image,album):
  topic = "/album/"+user+"/"+image+"/"+album+"/topic.json"
  fln = "/mnt/ebs0/imagediverdev/static/topic"+topic
  s3path = "/topic"+topic
  k = boto.s3.Key(bucket)
  k.key = s3path
  k.set_contents_from_string("hello helllo") # seems a bug, but this is needed
  k.set_contents_from_filename(fln,headers={"Content-Type":"application/json"})
